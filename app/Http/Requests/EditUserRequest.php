...
class EditUserRequest extends FormRequest
{
    ...
    public function rules()
    {
        if ($this->request->has('classes_id') || $this->request->has('workplace') || $this->request->has('year_of_finish') || $this->request->has('status')) {
            return [
                ...
                'classes_id' => 'required',
                'year_of_finish' => 'nullable|digits:4|integer|min:2000|max:' . (date('Y') + 5),
                'status' => 'required',
            ];
    ...
    public function messages()
    {
        return [
            ...
            'year_of_finish.max' => 'A legnagyobb megadható végzési év ' . (date('Y')) . '!',
            'status.required' => 'Az státusz megadása kötelező!',

            'degree.required' => 'A fokozat megadása kötelező!',
